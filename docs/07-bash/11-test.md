...
* The `-f` and `-d` tests are lowercase, but the `-L` test is UPPERCASE.

* If a symbolic link points to a file, then both the `-f` and `-L` tests will succeed. If a symbolic link points to a directory, then both the `-d` and `-L` tests will succeed.
